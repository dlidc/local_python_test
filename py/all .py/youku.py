# !/usr/bin/env python3
# encoding: utf-8
__author__ = 'PiZhong Zhou (zpz@zpz.me)'
...
_magic_1 = b'becaf9be'
_magic_2 = b'bf7e5f01'
_info_url = 'http://v.youku.com/player/getPlayList/' \
            'VideoIDS/%s/Pf/4/ctype/12/ev/1'
_m3u8_url = 'http://pl.youku.com/playlist/m3u8?' \
            'ctype=12&ep=%s&ev=1&keyframe=1&oip=%s&' \
...
def get_youku_video(url_or_id, types=None):
    ...
    sid, token = _encode(_magic_1, decodebytes(info['ep'].encode())).split(b'_', 1)
    ep = quote(encodebytes(_encode(_magic_2, sid + b'_' + url_or_id.encode() + b'_' + token))[:-1])
    token = token.decode()
    sid = sid.decode()
